...
class PointCloudConverterNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("LiDAR 감지 노드 & 마커 퍼블리셔 준비 완료!")
    ...
    def lidar_callback(self, msg: PointCloud2):
        ...
        try:
            ...
            z_values = points_xyzi[:, 2]
            current_ground_z = np.percentile(z_values, 5) # 하위 5% 지점을 바닥으로 추정
            
            # 바닥 높이를 KITTI 기준인 -1.73m가 되도록 전체 포인트를 위아래로 이동시킵니다.
            ...
            # ---------------------------------------------------------
            # 3. [추가] Rviz 마커 생성 및 퍼블리시
            # ---------------------------------------------------------
            if len(pred_boxes) > 0:
                self.publish_markers(pred_boxes, msg.header)
    ...
    def publish_markers(self, boxes, header):
            ...
            CLASS_COLORS = {
                ...
                2: (1.0, 0.0, 0.0),    # Pedestrian (Red)
                3: (0.0, 1.0, 1.0),    # Cyclist (Cyan)
            }
            ...
            for i, box in enumerate(boxes):
                ...
                # 텍스트 내용 구성
                label_str = LABEL_MAP.get(class_id, f"ID:{class_id}")
                
                text_marker.text = f"{label_str}\n({score:.2f})"
